...
\usepackage{xurl}
\hypersetup{pdftitle={柔性桁架振动与目标位姿不确定条件下空间机器人柔顺组装方法研究},pdfsubject={南航重点实验室开放课题申请书报告正文}}
\setstretch{1.32}
...
\begin{center}
 {\zihao{3}\bfseries 柔性基座空间机器人的受约束阻抗控制与接触引导组装研究\par
\par}
 \vspace{0.55em}
...
此外，上述耦合系统具有高维数、多时间尺度和模型不确定性等特征，基于高维模型在线求解结构力学响应，难以兼顾动力学描述精度与反馈控制的实时性。面向在轨作业的测控条件，需要提取影响末端运动与接触响应的主要耦合信息，降低控制对完整结构模型和高维动力学求解的依赖，并结合运动与接触反馈调节控制作用，以适应支撑运动及接触状态变化。

针对上述问题，本项目拟以柔性桁架支撑的空间机器人为研究对象，研究面向实时控制的耦合动力学表征与受约束几何阻抗控制方法，利用接触几何信息和接触反馈实现接口对准与柔顺组装，并通过地面试验考核验证。研究有助于阐明柔性支撑与多源不确定性共同作用下的末端运动和接触响应规律，形成兼顾实时性、运动精度与接触柔顺性的控制方法，为大型空间天线、太空望远镜等装备的机器人在轨组装提供理论依据与技术支撑。
...
机器人依附柔性桁架作业时，机械臂运动通过安装端反作用力与力矩激发支撑振动（这儿的支撑其实就是柔性桁架，那为什么要用支撑来表述呢，我觉得支撑这个表述不太好），支撑振动又经基座运动影响末端位姿与动态响应。针对上述双向耦合及关节驱动能力受限的问题，从安装界面的运动与受力出发，建立面向实时控制的耦合动力学描述，研究位置与姿态协同的受约束阻抗控制方法，为振动环境下的精确运动与接触作业提供基础。主要研究内容如下：
...
面向一种已有离线接触数据的典型接口，在目标固定、局部接触区域及预定小幅基座激励下，研究接触观测误差与参考修正对组装响应的影响。以已有接触场配准和约束导纳捕获方法为基础，完成与阻抗--QP的衔接及对准、插接和到位保持验证。主要研究内容如下：
\begin{enumerate}[label=\arabic*)]
 \item 统一接触观测坐标，分析基座测量误差、时序偏差及接触分布对六维配准的局部影响，识别所选接口中约束较弱的位姿方向，为坐标修正与目标限幅提供依据。
 \item 沿用六维配准与五轴约束导纳，保留修正通道的零输入保持性质和独立轴向接近，研究修正速率、偏移限幅与下层跟踪能力的匹配，减轻参考变化引起的接触载荷。
 \item 在预定基座激励和初始偏差组合下，评价目标估计、参考修正及受约束执行对到位误差和接触载荷的影响，报告所测试工况中的成功与失败结果。
\end{enumerate}

\topic{（3）柔性支撑机器人运动与组装的地面试验验证}

依托KUKA末端悬挂Franka的实验平台，由KUKA施加基座运动激励，模拟柔性支撑振动引起的安装端运动，Franka执行运动与组装任务。围绕不同形式基座运动、末端负载与初始位姿偏差条件，开展运动控制与典型接口组装的分项及集成试验，验证所提动力学描述与控制方法，明确其适用范围。主要研究内容如下：
\begin{enumerate}[label=\arabic*)]
 \item 完善KUKA与Franka之间的固定连接、典型接口与同步测量系统，开展基座及末端位姿、安装端及接触端力和力矩的标定，设置不同幅值、频率和方向的基座运动激励，建立可重复的基座振动模拟与试验评价方法。
...
面向大型空间结构在轨组装中柔性支撑机器人的精确运动与柔顺操作需求，建立基于安装界面运动与受力的耦合动力学描述，揭示支撑振动、关节驱动约束与末端运动响应之间的关系，形成兼顾位置、姿态及驱动约束的几何阻抗控制方法；明确典型接口局部接触观测误差与参考修正对组装响应的影响，形成接触配准、约束导纳修正与阻抗--QP相衔接的方法，完成预定基座激励和初始偏差工况下的对准、插接与到位保持原理验证；依托KUKA末端悬挂Franka的地面实验平台，验证所提动力学描述与控制方法的有效性，明确其在不同振动、末端负载及初始位姿偏差条件下的适用范围，形成理论分析、数值仿真与地面试验相互印证的研究框架。
...
需要通过局部误差分析与对照试验，阐明观测误差向目标估计的传递，以及参考修正速率、偏移限幅和执行偏差对到位误差与接触力峰值的共同影响，为有限基座激励下的接触观测处理和参考参数选取提供依据。

\subsection{拟采取的研究方案及可行性分析}

\subsubsection{总体研究思路与技术路线}

对应三项研究内容，按照“耦合动力学与受约束运动控制、接触引导柔顺组装、地面试验验证”的顺序实施。首先以作业端为浮动基建立倒置机械臂动力学模型，通过实际安装端的支撑力和力矩计入柔性支撑作用，结合$\SE$几何阻抗和动力学二次规划（QP）实现受约束力矩控制；进一步利用接触流形的几何约束，通过接触场配准估计组装目标，并以有界参考修正实现柔顺对准与插接；最后利用KUKA末端悬挂Franka的平台模拟基座振动，开展分项对照与集成验证。技术路线如图\ref{fig:route}所示，各项方案分别说明如下。
...
\begin{tikzpicture}[
 ...
 \node[block,fill=blue!8] (need) {\textbf{柔性桁架支撑下的精确运动与柔顺组装}\\[3pt]
 基座振动\qquad 关节驱动约束\qquad 目标位姿偏差};
 \node[block,below=of need] (control) {\textbf{研究内容一\quad 耦合动力学与受约束运动控制}\\[3pt]
 倒置动力学\quad $\SE$几何阻抗\quad 动力学QP};
...
\end{equation}
其中，$\bm M$、$\bm C$和$\bm G$分别为名义惯性矩阵、科氏及离心项矩阵和重力项；$\bm F_b$为作用在浮动基即实际作业端的接触力旋量，$\bm F_e$为作用在虚拟末端即实际安装端的支撑力旋量，$\bm J_g$为其几何雅可比矩阵。自由运动时$\bm F_b=0$，接触阶段由测量或经校核的估计获得；$\bm F_e$以满足浮动基重力平衡的静态支撑力为名义值，支撑运动引起的动态支撑力偏差结合实测数据进行评价。$\bm\tau_j$为待求解的命令等效关节力矩。
...
在QP可行及状态估计误差有界的条件下，分析安装端支撑作用偏差对位姿与速度误差动态的影响，研究局部闭环稳定与误差有界的条件。进一步考察约束激活时参考加速度松弛对实际阻抗响应的影响，结合基座运动、安装端受力及执行时序的实测数据校核扰动范围，并通过参数变化与约束激活试验检验分析结论的适用性。
...
该更新满足有界修正与参考端口离散广义功不等式。将修正位姿交由阻抗--QP执行，考察修正参数、跟踪误差与参考加速度松弛对到位精度和接触峰值的影响。理论结论限于局部误差传播和参考子系统，整机效果由对照试验评价。
...
对应第一项研究内容，先以位姿保持和圆轨迹校核倒置动力学、$\SE$阻抗和QP控制链，对比数值仿真与实测响应，考察不同基座振动、末端负载及约束激活时的位姿误差、力矩边界与柔顺响应。记录位置均方根误差、旋转测地误差、关节力矩幅值与变化率，以及安装端力和力矩；同时记录控制计算耗时的分布、最大值及超出控制周期的比例，验证所提方法的实时执行能力。相同工况下保持KUKA激励指令、Franka任务参考、执行时间与驱动约束一致，并核查实际基座运动的一致性，至少进行5次重复试验；参数对照在代表性工况开展。

对应第二项研究内容，采用一种典型接口和一种代表性载荷，将目标固定于独立刚性基准。先在基座静止时开展基础柔顺控制、仅配准、仅接触参考修正及二者结合的四组对照，再选取少量平移或转动激励重复关键对照。统一初始偏差、执行预算和保持窗口，记录目标估计误差、独立测量的到位误差、达标次数、接触峰值及安装端受力。工况与判据结合接口容差、测量精度和预实验结果，在正式对照前固定；成功与失败试验均纳入评价，调参数据不计入独立验证。

对应第三项研究内容，完成上述分项试验及运动与组装的集成验证，围绕基座振动、末端负载与目标初始位姿误差设置对照，分析其对末端运动精度和接触对准的影响，明确各控制环节的贡献及方法适用范围。对于按给定运动施加的基座激励，以同等激励下的末端误差和接触响应评价控制效果，同时报告绝对误差与重复试验离散程度；安装端反作用力与力矩用于分析控制对支撑的作用，其变化不直接等同于柔性结构振动幅值的变化。

\subsubsection{可行性分析}

\topic{（1）控制方法与实验接口具备直接基础}

现有位姿保持与圆轨迹实验具备倒置模型、几何阻抗及QP控制链。KUKA末端悬挂Franka的实验平台可通过安装端运动施加振动扰动，支持几何微分、坐标映射与基座激励的校核，并可在此基础上开展支撑作用偏差及驱动约束影响分析，具备分步实施基础。

\topic{（2）接触研究与地面条件能够支撑方法衔接}

已有PCF配准与RAC捕获方法已完成仿真、真机配准回放及多轮对接验证，可直接作为本项目的接触算法基础。现有真机采用导纳与位置伺服，本项目重点完成其与阻抗--QP力矩控制的衔接，并由基座静止逐步扩展至KUKA施加振动的工况。两机械臂及接口条件具备，需完善测点、同步采集与系统集成。

\topic{（3）研究范围与实施难点相匹配}

研究以Franka作为作业机械臂，KUKA承担基座运动激励，试验限定于预定振动范围与典型接口。利用实测基座运动与安装端受力校核动力学描述，结合末端误差和关节力矩调整阻抗参数与约束设置，利用先验与修正边界应对接触信息不足。各环节分别验证后再集成，使建模、控制和试验工作与两年开放课题条件相适应。

\subsection{本项目的特色与创新之处}

\topic{（1）面向柔性安装界面建立动力学一致的阻抗控制方法}

在已有倒置建模基础上，以作业端为浮动基、实际安装端为虚拟末端，将作业端运动与安装端支撑力纳入同一动力学描述，通过运动反馈与受约束力矩分配实现期望阻抗响应。拟阐明支撑作用偏差与驱动约束对实际阻抗响应的共同影响，形成降低在线结构力学求解依赖的柔性支撑机器人控制方法。



\topic{（2）接触几何与实际阻抗响应协调的柔性支撑对接}
...
\topic{2027年1月至6月}
围绕研究内容一开展倒置动力学与支撑作用的建模研究，并启动研究内容三的试验条件完善。确定KUKA基座激励与末端负载工况，完善Franka悬挂连接及接口夹具，完成运动测量、力和力矩传感器及执行时序标定；校核倒置模型中作业端运动、关节驱动与安装端支撑力之间的关系，复核现有控制接口。阶段成果为动力学模型、标定数据及基线试验方案。

\topic{2027年7月至12月}
完成研究内容一的$\SE$阻抗与动力学QP控制设计及初步闭环分析，开展不同基座振动与末端负载条件下的位姿保持、轨迹跟踪试验，明确支撑作用偏差和约束激活对运动与阻抗响应的影响；启动研究内容二的基座运动与接触观测同步校核，检验已有接触场在动态工况下的适用性。形成论文初稿1篇、年度进展报告，并与重点实验室开展专题交流。

\topic{2028年1月至6月}
围绕研究内容二完成已有PCF与RAC的基座坐标修正及阻抗--QP衔接，开展局部误差与参考参数影响分析，完成基座静止时的四组方法对照，并在预定单向激励下验证，形成方法论文和发明专利申请材料。
...
对应三项研究内容，形成柔性支撑机器人倒置耦合动力学与$\SE$阻抗--QP程序1套、典型接口的接触配准与参考修正模块1套，以及KUKA--Franka平台的基座振动工况、同步数据和试验流程；完成振动扰动下的机器人运动与典型接口组装地面原理验证，提交技术总结报告和试验测试报告各1份。围绕新增研究发表或录用学术论文2篇，申请发明专利1项，培养参与研究的研究生2名。
...
本项目采用现有KUKA机械臂末端悬挂Franka机械臂的实验平台，通过KUKA运动模拟Franka安装端受到的振动扰动。现有Franka开放力矩控制程序、动捕和IMU状态输入与同步记录接口，可支持运动控制集成与对照实验；已有模块对接平台、六维力/力矩测量及接触记录，可支撑接口标定和柔顺接触研究。FR3阻抗--QP程序及接触记录回放流程为运动控制实现与接触对准方法验证提供了基础。
...
\textbf{李融冰：}拟承担FR3控制程序集成及真机实验，重点开展实时力矩接口实现、基座振动下运动控制验证和实验数据分析。
