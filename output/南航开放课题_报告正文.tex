...
% 地面实验采用KUKA末端悬挂Franka的构型，以基座运动模拟柔性支撑振动。
...
大型空间天线、太空望远镜等装备的发展，对空间设施的尺度、功能与建造精度提出了更高要求。受运载包络和发射能力限制，依靠单次整体发射与在轨展开难以满足不断增长的建造需求，通过模块分批运输、机器人在轨转运与装配逐步形成大型设施，成为拓展空间装备能力的重要途径\cite{hu2025,wu2024}。在这一过程中，空间机器人需要依附已建结构，连续完成模块拾取、转运、接口对准和安装等操作。支撑结构运动、搬运载荷变化及接触状态转换，使机器人面临复杂的动力学条件与精密装配要求，传统基于固定基座和理想动力学模型的控制方法难以兼顾运动精度与接触柔顺性。因此，亟需发展适应动态支撑与多源不确定性的机器人运动与组装控制方法，满足大型空间设施自主、精密、可靠建造的需求。

其中，机器人运动与支撑结构振动的双向耦合，是影响运动精度与组装稳定性的突出问题。对于柔性桁架支撑的多自由度机器人，关节反作用载荷激励桁架振动，结构振动引起机器人基座的位置和姿态变化，并经机械臂传递至作业端，影响末端定位与接口对准；惯性参数、负载、摩擦及执行时延等偏差进一步增加精确控制的难度。单纯依靠提高反馈增益减小运动误差，难以同时满足结构振动抑制与接触载荷调节的要求。尤其在接口对准与插接阶段，较高的控制刚度限制了机器人顺应接触约束的调整能力，目标位姿偏差与支撑振动易引起接触载荷增大，导致接口卡滞、冲击过载等问题。因此，需要揭示支撑运动、动力学失配与接触作用对末端响应的影响规律，协调运动精度、振动抑制与接触柔顺性，实现振动环境下的精确对准与平稳插接。
...
在轨组装使空间机器人的作业对象由独立航天器拓展至大型柔性结构，动力学研究也由基座与机械臂的运动耦合，进一步延伸至机器人运动、结构变形与组装接触的相互作用\cite{papadopoulos2021,hu2025,wu2024}。相关研究主要沿两条思路开展：一是通过模型降阶保留主要动力学特征，降低耦合系统的计算成本；二是将耦合模型引入运动规划与控制，减小机器人操作对结构的扰动。
...
机器人在柔性结构上作业，需要同时控制末端运动与结构振动。已有方法主要通过调节机械臂的反作用载荷，或利用柔性状态反馈补偿振动影响。前者以Nenchev等提出的反作用零空间控制为代表，通过利用机械臂与支撑结构的耦合关系，协调末端路径跟踪与基座振动抑制\cite{nenchev1999}；后者如Patel和Damaren针对柔性空间机械臂的研究，将柔性坐标估计用于末端与振动控制\cite{patel2024}。两类方法分别体现了运动协调和状态反馈对控制结构振动的作用。

当作业进入接口对准与插接阶段，控制目标还需兼顾接触载荷与装配精度。阻抗控制通过设定末端运动偏差与接触力之间的动态关系，调节机器人与环境接触时的柔顺特性。针对位置与姿态的统一描述，Kim等利用$\SE$与其李代数之间的映射构建六自由度几何阻抗，明确了位姿误差与控制作用之间的转换关系\cite{kim2025}。面向在轨模块组装，史玲玲等根据接触力跟踪误差在线调整柔顺轨迹，并通过地面实验验证了自适应阻抗控制对复杂对接工况的适应能力\cite{shi2024}。相关研究由运动跟踪进一步拓展至接触过程中的力与运动协调。

然而，期望阻抗关系的实际实现依赖机器人动力学与关节驱动。柔性支撑条件下，关节力矩既用于控制末端运动，也通过反作用激励结构振动；支撑结构本身不能由关节驱动直接、独立地控制。当关节运动范围或驱动力矩达到限制时，实际末端响应将偏离所设计的阻抗关系。因此，需要在运动与振动协调的基础上，明确驱动限制和支撑运动对柔顺响应的影响。此外，接触载荷减小并不意味着接口已准确对准，仍需结合接触几何信息修正目标位姿，协调接触顺应与装配精度。
...
对于柔性支撑下的阻抗控制，残差补偿还需区分模型失配造成的运动偏差与顺应接触所需的柔顺运动。若仅以减小跟踪误差为学习目标，补偿作用可能改变预定的阻抗特性，并增加支撑结构的受激响应。因此，需要将动力学偏差补偿与阻抗设计、驱动约束相结合，明确学习残差对运动精度、接触载荷和结构振动的共同影响。

综上，相关研究已在耦合建模、运动振动协调、柔顺接触与学习补偿方面取得进展。面向柔性支撑下的精确组装，仍需贯通主要耦合作用的实时表征、动力学失配下的阻抗响应保持，以及目标位姿不确定条件下的接触对准，并通过地面实验检验各环节相互作用下的整体控制性能。
...
\begin{tikzpicture}[
 ...
 \node[wide] (control) at (0,-3.2) {\textbf{学习增强的力矩控制}\\振动扰动下的位姿保持、轨迹跟踪与柔顺响应};
 \node[wide] (contact) at (0,-4.8) {\textbf{内容三：接触对准与柔顺组装}\\接触流形表征\quad 接触场配准\quad 有界参考修正};
 \node[wide] (test) at (0,-6.4) {\textbf{内容四：地面试验验证}\\KUKA末端悬挂Franka，模拟基座振动\\运动精度\quad 安装端载荷\quad 接触载荷\quad 组装到位精度};
...
沿用以作业端为浮动基的倒置动力学模型，将实际安装基座作为倒置运动链的虚拟末端，通过该处的支撑力和力矩描述柔性桁架对机械臂的作用。结合关节状态与作业端的绝对位姿、速度测量，确定倒置模型的运动状态，使支撑运动对末端的影响能够通过实际运动反馈进入控制过程。在此基础上，以几何阻抗关系生成作业端的参考加速度，通过动力学QP协调末端运动要求、关节运动与驱动能力。支撑作用的计算以现有静态载荷近似为基线，结合安装端运动和载荷数据校核其在动态工况下的适用性，分析支撑作用偏差对实际控制响应的影响。

在地面装置中，Franka基座与KUKA末端连接，该连接处对应模型中的实际安装端；Franka执行作业的末端对应倒置模型的浮动基。KUKA提供安装端运动激励，其运动记录与安装端载荷用于校核支撑作用，Franka的关节力矩由所提控制方法生成。由此建立在轨支撑作用、地面基座激励与控制模型之间的对应关系。
...
\end{equation}
其中，$\bm M$、$\bm C$和$\bm G$分别为名义惯性矩阵、科氏及离心项矩阵和重力项；$\bm F_b$为作用在浮动基即实际作业端的接触力旋量，$\bm F_e$为作用在虚拟末端即实际安装端的支撑力旋量，$\bm J_g$为其几何雅可比矩阵。自由运动时$\bm F_b=0$，接触阶段由测量或经校核的估计获得；$\bm F_e$以满足浮动基重力平衡的静态支撑力为名义值，支撑运动引起的动态载荷偏差结合实测数据进行评价。$\bm\tau_j$为包含补偿的总命令等效关节力矩，$\bm d$为求解前给定的学习残差，未引入学习时取零。
...
在包含基座运动的机器人动力学仿真中随机化惯性、摩擦、载荷与时序，并以KUKA--Franka平台实测的基座运动和安装端载荷校核训练工况。已有方法利用微分动态规划（Differential Dynamic Programming，DDP）在特定轨迹上的局部价值信息引导学习；本项目面向阻抗--QP控制，根据阻抗误差、安装端载荷及补偿代价重构训练目标。保留双端点输出和平滑执行，以适应不同控制频率；真机采用冻结策略推理，批次之间更新策略。

在名义闭环局部稳定、QP可行及估计误差有界的条件下，以位姿与速度误差为对象，分析有界学习残差及支撑作用偏差对闭环跟踪响应的影响，研究误差最终有界的充分条件。结合安装端载荷、状态估计与执行时序的实测偏差，校核分析中采用的扰动范围，并通过约束激活与参数变化试验检验结论的适用性；基座振动下的末端运动和接触响应通过仿真与地面试验评价。
...
分析局部接触参考更新的储能变化，进一步考察下层跟踪误差与支撑运动对载荷和对准的影响。参考子系统结论的整机适用性，以及有限转动和接触变化的影响，由包含基座运动的仿真与实验评价。

\subsubsection{地面试验系统与分项、集成验证}

依托KUKA末端悬挂Franka的装置及现有测量系统，由KUKA驱动Franka基座产生可重复的平移或转动激励，模拟柔性支撑振动在机器人安装端形成的运动；Franka执行位姿保持、轨迹跟踪及典型接口组装。依据平台工作范围和预实验结果，按激励幅值、频率与方向设置3组基座振动工况，并配置2种作业载荷；以KUKA静止时的悬挂构型作为补充基线，比较基座运动对作业性能的影响。

同步采集Franka关节状态、实际基座运动、作业端位姿、安装端载荷及接触力和力矩，完成悬挂连接外参、载荷测量和两机械臂执行时序标定。分别记录KUKA指令与Franka基座实测运动，以实测结果校核激励幅值、频率和相位；采用独立于控制反馈的测量结果评价末端精度与组装到位误差。安装端载荷按统一坐标表达，并结合重力和静态载荷标定分析其动态变化。

对应第一项研究内容，先以位姿保持和圆轨迹校核倒置动力学、$\SE$阻抗和QP控制链，考察不同基座振动及约束激活时的位姿误差、力矩边界与柔顺响应；记录控制计算耗时的分布、最大值及超出控制周期的比例，验证所提方法的实时执行能力。对应第二项研究内容，在3组基座振动工况与2种作业载荷组合下对比名义控制和残差学习，评价轨迹跟踪与位姿保持的位置均方根误差、旋转测地误差、安装端载荷及力矩变化。各组对照采用相同KUKA激励指令、Franka任务参考、执行时间与驱动约束，并核查实际基座运动的一致性，至少进行5次配对重复；关键消融在代表性工况开展。

对应第三项研究内容，在典型接口及不同初始偏差下，对比固定参考、配准后固定参考及配准结合接触修正，评价接触流形表征对目标估计和组装对准的作用。在集成试验中进一步对比开启与关闭学习补偿的结果，检验运动精度提升对接触对准及插接过程的贡献。记录目标估计误差、接触峰值、真实到位误差、作业完成情况及安装端载荷。目标安装方式在正式试验前固定，独立测量其在统一参考系中的位姿，按同时刻的末端与目标位姿评价相对对准误差；初始偏差与装配容差依据接口及预实验确定，调参和模型校核数据不计入独立验证。

地面试验重点检验基座振动向末端运动与接触响应的传递及其控制效果。通过记录悬挂连接状态、地面重力作用和KUKA激励跟踪偏差，说明试验工况与在轨作业条件的对应范围。对于按给定运动施加的基座激励，以同等激励下的末端误差和接触响应评价控制效果；安装端反作用载荷用于分析控制对支撑的作用，其变化不直接等同于柔性结构振动幅值的变化。机器人反作用驱动结构运动的双向耦合效应，应结合支撑动力学关系另行校核。

\subsubsection{可行性分析}

\topic{（1）控制方法与实验接口具备直接基础}

现有位姿保持与圆轨迹实验具备倒置模型、几何阻抗及QP控制链；Panda残差学习实验提供模型引导训练与分频部署经验。KUKA末端悬挂Franka的构型可通过安装端运动施加振动扰动，先校核几何微分、坐标映射与基座激励，再扩展学习目标，具备分步实施基础。
...
研究以Franka作为作业机械臂，KUKA承担基座运动激励，试验限定于预定振动范围与典型接口。通过实测基座运动和载荷校核改善策略迁移，结合末端误差与安装端载荷调节补偿幅值和变化速率，利用先验与修正边界应对接触信息不足。各环节分别验证后再集成，使建模、训练和试验工作与两年开放课题条件相适应。
...
将已有DDP引导残差学习拓展至随状态和约束变化的阻抗--QP系统，利用基座运动、安装端载荷和任务误差组织学习，将有界残差纳入统一力矩预算，并结合双端点执行研究不同更新频率下的末端响应与反作用载荷。拟建立学习补偿与名义阻抗的相容条件，在改善振动扰动下末端精度的同时保持所需柔顺性。
...
\topic{2027年1月至6月}
围绕研究内容一开展倒置动力学与支撑作用的建模研究，并启动研究内容四的试验条件完善。确定KUKA基座激励与作业载荷工况，完善Franka悬挂连接及接口夹具，完成运动、载荷测量和执行时序标定；校核倒置模型中作业端运动、关节驱动与安装端支撑力之间的关系，复核现有控制和残差接口。阶段成果为动力学模型、标定数据及基线试验方案。

\topic{2027年7月至12月}
完成研究内容一的$\SE$阻抗与动力学QP控制及初步闭环分析，开展名义控制分项试验；围绕研究内容二建立包含基座运动与安装端载荷的残差训练环境，研究状态、奖励及辅助监督设计，完成基座静止与KUKA施加振动条件下的对照。形成论文初稿1篇、年度进展报告，并与重点实验室开展专题交流。

\topic{2028年1月至6月}
完成研究内容二的残差策略与动力学QP融合，开展基座振动下的运动对照试验及局部闭环分析；围绕研究内容三建立接触流形表征、接触场配准与有界参考修正方法，完成基座静止时的组装控制集成，形成方法论文和发明专利申请材料。

\topic{2028年7月至12月}
完成研究内容三的基座振动条件下组装控制，并集中完成研究内容四的多工况重复试验与系统集成验证。完成基座振动与作业载荷组合下的运动对照及不同初始偏差下的典型接口组装试验，明确各环节贡献和方法适用范围；整理程序、数据及测试报告，完成论文发表或录用、专利申请和结题材料，与重点实验室开展成果交流。
...
团队已围绕七自由度Franka FR3形成以作业端为浮动基、通过实际安装端支撑力描述外部作用的倒置动力学实现，并建立$\SE$几何阻抗与动力学QP相结合的控制程序。该程序利用关节状态、动捕和IMU输入生成运动参考，在关节运动、力矩及其变化约束下求取控制指令，已具备位姿保持、圆轨迹及固定基座轨迹实验入口和数据记录、回放流程。现有支撑基线采用静态载荷近似，为本项目在KUKA施加基座振动的条件下校核动态支撑作用、研究末端运动与安装端载荷的关系提供了可扩展的实现基础。
...
表\ref{tab:preliminary}列出未参与训练轨迹集合的代表性结果，体现了残差补偿改善模型失配下执行精度的可行性。末端指标为平均误差，关节指标为均方根误差；本项目将另行按照末端均方根误差及安装端载荷指标评价基座振动工况。
...
上述研究提供了模型引导训练、负载变化测试和多频率力矩执行的经验。本项目将在此基础上面向阻抗--QP重构残差补偿方式，验证其对基座振动条件下运动精度、阻抗响应和安装端反作用载荷的影响。
...
本项目采用现有KUKA机械臂末端悬挂Franka机械臂的实验构型，通过KUKA运动模拟Franka安装端受到的振动扰动。现有Franka开放力矩控制程序、动捕和IMU状态输入与同步记录接口，可支持运动控制集成与对照实验；已有模块对接平台、六维力/力矩测量及接触记录，可支撑接口标定和柔顺接触研究。Panda残差学习真机实验和FR3阻抗--QP程序分别提供了学习策略部署与名义控制实现基础。

依托单位另建有约$4\,\mathrm m\times10\,\mathrm m$的在轨操作微重力模拟地面实验平台，可为相关地面验证提供配套条件。本项目悬挂装置的实验结果按照地面重力下的基座振动工况进行分析。

\topic{（2）尚需完善的条件及解决途径}

围绕基座振动下的闭环验证，拟完善KUKA与Franka之间的悬挂连接法兰、转接组件及标准载荷，布置基座运动与安装端载荷测点，完成载荷及位姿测量标定、两机械臂执行时序同步和传感器空间外参标定。结合现有对接平台完善典型接口夹具与独立位姿测量，使基座激励、末端运动和最终对接精度能够分别评价。连接组件、接口改装与专项测试按项目预算落实，计算与通用测量设备优先共享。

拟依托重点实验室在航空航天结构力学及控制方面的研究条件，围绕安装端支撑作用校核、运动与载荷测量及控制实验开展联合讨论和测试，按阶段研究进度落实设备共享与试验安排。
...
\textbf{苏文康：}已开展自由漂浮空间机器人任务优先协同操作与实时优化控制研究。拟承担几何阻抗与动力学QP控制设计、驱动约束协调及基座振动下的末端运动与安装端载荷分析。
...
设备费 & 4.00 & 同步采集和实时控制接口改装所需设备及配件1套，按4.00万元测算，用于两机械臂状态、基座运动、末端及载荷测量的时序同步。 \\
材料费 & 6.00 & 悬挂连接法兰、转接组件及配套材料按3.60万元测算；典型接口夹具及标准载荷材料1.40万元；传感安装、线缆等耗材1.00万元。 \\
测试化验加工费 & 9.00 & 悬挂连接及接口夹具加工装配3批，每批1.50万元，共4.50万元；基座运动、安装端载荷与位姿测量标定3批，每批0.80万元，共2.40万元；外协动态测量与联合测试3批，每批0.70万元，共2.10万元。共享仪器免费使用部分不重复计费。 \\
