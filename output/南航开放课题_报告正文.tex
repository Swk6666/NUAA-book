...
在组装执行层面，柔顺运动与主动接触交互的结合，使机器人能够在运动过程中逐步定位孔位、修正插接方向，并利用环境约束提高对位姿不确定性的适应能力\cite{chen2025}。结合上述估计方法可以看出，接触既提供运动约束，也提供目标修正所需的信息，控制过程需要兼顾信息获取与受力调节。对柔性桁架上的机器人而言，这一关系更加复杂：接触位姿变化可能同时来自目标偏差、基座振动与机器人柔顺响应；用于估计目标的探测运动和参考更新，又会改变接触力及安装端反作用。因此，将已有方法用于振动条件下的组装，需要在统一参考系中分析接触信息的可辨识性，并协调目标修正速度与实际阻抗响应。
...
\begin{equation}
 ...
 \label{eq:capture}
\end{equation}
其中，$\bm f_{c,k}$为与修正坐标共轭的接触作用，$\bm D_c$为正定阻尼权重，$\bm K_c$依据配准敏感程度设置并正则化为正定的参考保持权重，$\mathcal C_k$限定累计偏移和单步变化。将修正量通过位姿映射生成新的$T_d$，交由阻抗--QP控制器执行。两处优化分别负责接触参考调整与关节力矩分配，保持功能和物理量定义清楚。
...
对应第二项研究内容，在典型接口及不同初始偏差下，对比固定参考、配准后固定参考及配准结合接触修正，评价接触流形表征、目标配准和参考修正对组装对准的作用。由基座静止逐步过渡至KUKA施加振动的工况，记录目标估计误差、接触峰值、真实到位误差、作业完成情况及安装端力和力矩。目标安装方式在正式试验前固定，独立测量其在统一参考系中的位姿，按同时刻的末端与目标位姿评价相对对准误差；初始偏差与装配容差依据接口及预实验确定，调参和模型校核数据不计入独立验证。
...
在接触场配准与有界参考修正基础上，研究柔性支撑运动及下层执行偏差对接触对准的影响，将几何估计、接触反馈与受约束阻抗控制相联系，形成兼顾目标对准与柔顺接触的典型接口对接方法。
...
\topic{2027年7月至12月}
完成研究内容一的$\SE$阻抗与动力学QP控制设计及初步闭环分析，开展不同基座振动与末端负载条件下的位姿保持、轨迹跟踪试验，明确支撑作用偏差和约束激活对运动与阻抗响应的影响；启动研究内容二的典型接口接触数据采集与接触流形表征研究。形成论文初稿1篇、年度进展报告，并与重点实验室开展专题交流。

\topic{2028年1月至6月}
围绕研究内容二建立接触场配准与有界参考修正方法，分析接触参考更新与阻抗响应的协调关系，完成基座静止条件下的组装控制集成，并开展不同初始位姿偏差下的对准与插接试验，形成方法论文和发明专利申请材料。

\topic{2028年7月至12月}
完成研究内容二的基座振动条件下组装控制，并集中完成研究内容三的多工况重复试验与系统集成验证。完成基座振动与末端负载组合下的运动对照及不同初始偏差下的典型接口组装试验，明确各环节贡献和方法适用范围；整理程序、数据及测试报告，完成论文发表或录用、专利申请和结题材料，与重点实验室开展成果交流。

\subsubsection{预期研究成果与学术交流}

对应三项研究内容，形成柔性支撑机器人倒置耦合动力学与$\SE$阻抗--QP程序1套、接触流形表征与柔顺组装模块1套，以及KUKA--Franka平台的基座振动工况、同步数据和试验流程；完成振动扰动下的机器人运动与典型接口组装地面原理验证，提交技术总结报告和试验测试报告各1份。围绕新增研究发表或录用学术论文2篇，申请发明专利1项，培养参与研究的研究生2名。
