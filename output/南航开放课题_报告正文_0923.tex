...
% 可编译主文件；参考文献在本文件内，技术路线图采用用户更新的JPG图片。
...
\begin{enumerate}[label=\arabic*)]
  \item 研究桁架振动与机械臂运动的双向耦合机理，建立以作业端为浮动基的反向动力学模型，揭示基座振动向末端运动的传递规律及机械臂运动对安装端的反作用机制。
  \item 研究关节运动与驱动力矩约束下的$\SE$几何阻抗设计，通过动力学优化协调位姿跟踪与柔顺响应，形成适应基座振动的受约束运动控制方法。
  \item 研究桁架振动、负载变化与驱动约束对阻抗实现的影响，分析局部闭环稳定性与跟踪误差有界条件，阐明运动精度、接触柔顺性与安装端反作用之间的相互制约关系。
\end{enumerate}
...
针对振动环境下机器人运动精度与组装性能的验证需求，依托基座振动模拟与机器人作业实验平台，研究振动条件复现、多源同步测量与实验评价方法，通过运动控制对照实验和组装集成实验，检验耦合动力学模型、受约束控制及接触引导方法的有效性。主要研究内容如下：
\begin{enumerate}[label=\arabic*)]
  \item 研究不同频率与幅值下的基座振动复现方法，实现基座运动、末端位姿与接触力的同步测量，建立基于独立测量的运动精度评价方法。
  \item 开展静止基座与不同振动条件下的位姿保持、轨迹跟踪对照实验，分析基座振动与关节约束对运动精度和柔顺响应的影响，验证动力学模型的准确性及控制方法的实时性。
  \item 开展典型接口对准、插接与到位保持的组装集成实验，通过分项对照辨析接触配准与参考修正的作用，评价组装精度、接触力变化与组装成功率，明确方法在不同振动条件下的适用范围。
\end{enumerate}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{output/技术路线图/南航开放课题_技术路线图_重绘版.png}
\caption{项目技术路线与验证关系}
\label{fig:route}
\end{figure}
\FloatBarrier
...
以作业端为六自由度浮动基、实际安装端为虚拟末端，沿作业端至安装端重新组织运动链，建立倒置动力学模型：
\begin{equation}
  \bm M\dot{\bm\nu}+\bm h
...
\end{equation}
其中，$\bm\nu$由作业端体速度和关节速度组成，$\bm M$为惯性矩阵，$\bm h$包含科氏、离心及重力项；$\bm S$为关节选择矩阵，$\bm\tau_j$为关节驱动力矩；$\bm F_b$、$\bm F_e$分别为作用于作业端和安装端的界面力与力矩，$\bm J_b$、$\bm J_g$为对应雅可比矩阵。当前自由空间位姿保持实验取$\bm F_b=0$。

安装端载荷采用随构型更新的静态支撑近似：根据当前构型下的重力项和安装端雅可比，求解满足浮动基六维静力平衡的支撑载荷，并将其映射为广义力，作为动力学约束中的载荷基线。地面实验中，将模拟机械臂末端与作业机械臂安装端刚性连接，由模拟机械臂施加基座运动；作业机械臂以初始作业端世界位姿为固定目标，期望速度和加速度均设为零，通过位置与姿态误差评价基座运动下的位姿保持效果。

为将作业端位姿偏差转化为加速度参考，在$\mathrm{SE}(3)$上描述实际位姿$T_b$与期望位姿$T_d$，定义局部误差坐标$\bm\lambda=\operatorname{Log}(T_b^{-1}T_d)^\vee$，并在误差坐标中构造阻抗关系\cite{kim2025}：
\begin{equation}
  \bm A_\lambda\ddot{\bm\lambda}_{\mathrm{ref}}
  +\bm D_\lambda\dot{\bm\lambda}
  +\bm K_\lambda\bm\lambda
  =\bm\gamma .
  \label{eq:impedance}
\end{equation}
其中，$\bm A_\lambda$、$\bm D_\lambda$和$\bm K_\lambda$分别为误差坐标中的虚拟惯量、阻尼和刚度矩阵，$\bm\gamma$为按误差方向约定的外力输入，当前实验取零。由该关系计算$\ddot{\bm\lambda}_{\mathrm{ref}}$，再通过指数映射的微分及其时间导数转换为作业端六维参考加速度，并统一到动力学求解所用坐标。

将作业端参考加速度作为动力学二次规划（QP）的软跟踪目标，结合关节速度阻尼对应的加速度参考和力矩正则项，联合优化作业端加速度、关节加速度与驱动力矩。以倒置动力学为等式约束，施加关节加速度、力矩幅值及单周期力矩变化量限制，并加入关节位置约束，求得满足上述约束的关节力矩。
...
由模拟机械臂驱动作业机械臂基座产生可重复的平移与转动，模拟桁架振动引起的安装端运动。依据平台范围和预实验设置激励参数，通过末端配重调整负载，以静止基座为基线。同步采集基座与末端位姿、关节位置、速度及关节力矩，分析运动传递的幅值与相位。完成标定、外参测定及时序同步，采用独立于控制反馈的测量评价末端精度。

运动控制验证包括位姿保持、圆轨迹跟踪及接触扰动，结合仿真与实测，分析载荷近似和约束激活对运动响应的影响。统一参考与激励，核查基座实测运动，记录位置均方根误差、旋转测地误差、关节力矩及变化率，比较不同阻抗参数下的位姿响应；统计计算耗时分布、最大值及超周期比例。

组装验证采用典型接口及负载，目标固定于独立刚性基准。基座静止时设置基础柔顺控制、仅配准、仅参考修正及二者结合四组对照，在代表性激励下重复关键对照。统一初始偏差、执行时间及保持窗口，记录估计、限幅参考与实际到位结果，分离估计和执行误差，评价到位精度与达标次数。

集成验证贯通接近、对准、插接与到位保持，依据接口容差和测量精度预先确定工况及判据。成功与失败实验均纳入评价，调参数据不计入独立验证，统计到位位置与姿态误差的均值和标准差，记录组装成功率。通过静止基座与不同激励下的结果对照，评价基座运动对末端跟踪精度、组装成功率及到位保持效果的影响。
...
\textbf{耦合动力学建模方面：}团队已在自由漂浮双臂空间机器人协同操作研究中，建立包含浮动基座、机械臂与抓持目标的闭环系统运动学和动力学模型，描述基座与机械臂的动力学耦合及双臂协同操作约束\cite{su2026}。本项目沿用浮动基多体系统建模框架，以作业端为浮动基重新组织运动链，通过实际安装端的运动与载荷表征外部作用。前期已实现反向机械臂模型及安装端载荷的静力近似描述，可在此基础上结合实测基座运动与载荷校核动态偏差，为面向控制的耦合建模提供直接基础。

\textbf{机械臂运动与阻抗控制方面：}几何阻抗为末端位置与姿态的柔顺响应提供统一描述，动力学二次规划为运动目标与关节驱动能力的协调提供求解手段。团队已开展基于分层二次规划的任务优先级实时优化控制研究，积累了多任务协调与物理约束处理的方法基础\cite{su2026}；前期已进一步将几何阻抗与动力学优化结合，在关节运动和力矩约束下求取控制指令，并用于位姿保持和圆轨迹跟踪实验。已有控制算法与实验基础，可支撑本项目研究基座扰动和建模偏差对末端跟踪及柔顺响应的影响，并通过闭环误差分析与对照实验检验驱动约束下的阻抗可实现性。

\textbf{接触引导组装方面：}团队已形成物理方向辅助接触场配准（PCF）与约束导纳捕获（RAC）方法，利用接触几何与物理方向信息估计目标位姿，通过有界参考修正实现柔顺对准，并完成仿真与多轮真机对接实验。现有方法具备修正速率与累计偏移约束、零输入保持性质，并已建立参考端口离散广义功关系，为接触引导过程的误差分析与柔顺控制提供了依据。本项目可在已有方法上引入基座运动补偿，分析观测偏差与参考修正的误差传递，再由静止基座逐步扩展到振动工况，检验接触引导与机械臂力矩控制的协调效果，具备明确的方法基础和实验验证途径。

\topic{（2）实验方案与研究条件可行}

\textbf{实验方案：}模拟机械臂与作业机械臂的固定连接可形成可控、可重复的安装端运动环境，配重与接口夹具可设置负载和初始偏差。通过基座与末端位姿、关节状态及两端受力的同步观测，可分别评价模型误差、跟踪性能与接触响应；结合独立刚性基准和精度测量，能够对照检验各控制环节，覆盖主要研究问题。

\textbf{研究基础与条件看：}团队已有基座振动模拟与机器人作业平台、开放力矩控制程序、运动捕捉系统、IMU和六维力测量条件，并积累了多模块对接实验经验。拟完善的连接组件、测点、时序同步和接口夹具可依托现有设备、共享条件与项目预算落实。按单项模型与控制校核、接口对照和振动环境集成验证逐步推进，工作量与两年研究周期相匹配。
