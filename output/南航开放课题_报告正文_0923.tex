...
以作业端为浮动基、实际安装端为虚拟末端，沿作业端向安装端重新组织运动链，建立反向动力学模型，模型基本形式为：
...
其中，$\bm\nu$由作业端体速度和关节速度组成，$\bm M$与$\bm h$分别为惯性矩阵和包含科氏力与重力作用的偏置项；$\bm S$为关节选择矩阵，$\bm\tau_j$为关节驱动力矩；$\bm F_b$、$\bm F_e$及其雅可比矩阵$\bm J_b$、$\bm J_g$分别对应实际作业端和安装端。自由运动时$\bm F_b=0$，接触时由力传感测量计入。
...
在$\SE$上统一描述实际位姿$T_b$与期望位姿$T_d$，采用局部误差坐标\allowbreak$\color{black}\bm\lambda=\operatorname{Log}(T_b^{-1}T_d)^\vee$，将几何阻抗映射为如下的阻抗控制方程\cite{kim2025}：
\par\endgroup
...
其中，$\bm A_\lambda$、$\bm D_\lambda$、$\bm K_\lambda$为经几何映射得到的等效惯量、阻尼和刚度矩阵，$\bm\gamma$为与误差坐标及相对运动方向一致的广义接触输入。通过求解该阻抗控制方程可获得期望加速度$\ddot{\bm\lambda}_d$，并将其映射为作业端的期望加速度$\dot{\bm\nu}_d$，作为反向动力学的输入。

将参考加速度嵌入动力学二次规划（QP），以参考跟踪、关节虚拟阻尼和力矩正则为目标，以反向动力学为等式约束，统一计入关节运动范围、力矩及其变化率限制。
