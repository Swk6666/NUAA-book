% !TeX program = xelatex
% 2026-09-21 复核修订版：保留必要的研究论证与衔接修改，相对原稿标红。
% 题目及正文采用柔性桁架支撑等表述；撤回过细的新增技术承诺。
% 研究目标保持一段；关键科学问题名称、图1及公式保留原稿。
...
\begin{center}
 {\zihao{3}\bfseries \rev{柔性桁架支撑下空间机器人的运动控制与接触引导组装研究}\par
\par}
 \vspace{0.55em}
...
空间机器人依附柔性桁架作业时，其末端运动与接触响应受机械臂运动、柔性桁架变形及二者动力学耦合的共同影响。\rev{机械臂运动产生的反作用力与力矩}经基座传递至桁架并激发结构振动，桁架振动又引起机器人基座的位置和姿态变化，并经机械臂传递至作业端，影响末端定位与接口对准；惯性参数、负载、摩擦及执行时延等偏差进一步增加精确控制的难度。单纯依靠提高反馈增益减小运动误差，难以协调振动扰动抑制所需的控制刚度与接触过程所需的柔顺性。尤其在接口对准与插接阶段，较高的控制刚度限制了机器人顺应接触约束的调整能力，目标位姿偏差与桁架振动易引起接触力增大，引发接口卡滞或接触冲击。因此，需要揭示基座运动、动力学失配与接触作用对末端响应的影响规律，\rev{协调振动扰动下的运动精度与接触柔顺性}，实现振动环境下的精确对准与平稳插接。
...
针对上述问题，\rev{本项目以柔性桁架支撑的空间机器人为研究对象}，研究面向实时控制的耦合动力学表征方法，结合几何阻抗设计与动力学优化开展机器人运动控制，利用接触几何信息和接触反馈实现接口对准与柔顺组装，并通过地面试验考核验证。研究有助于阐明柔性桁架振动与多源不确定性共同作用下的末端运动和接触响应规律，形成兼顾实时性、运动精度与接触柔顺性的控制方法，为大型空间天线、太空望远镜等装备的机器人在轨组装提供理论依据与技术支撑。
...
在组装执行层面，柔顺运动与主动接触交互的结合，使机器人能够在运动过程中逐步定位孔位、修正插接方向，并利用环境约束提高对位姿不确定性的适应能力\cite{chen2025}。结合上述估计方法可以看出，接触既提供运动约束，也提供目标修正所需的信息，控制过程需要兼顾信息获取与受力调节。\rev{对柔性支撑条件下的机器人而言}，这一关系更加复杂：接触位姿变化可能同时来自目标偏差、基座振动与机器人柔顺响应；用于估计目标的探测运动和参考更新，又会改变接触力及安装端反作用。因此，将已有方法用于振动条件下的组装，需要分析观测误差对目标估计的影响，并协调参考修正速度与实际阻抗响应。

综上，相关研究已形成由耦合动力学分析、运动振动协调到接触估计与柔顺操作的方法基础。\rev{面向桁架振动条件下的机器人精确组装}，进一步研究的重点在于贯通这些环节：以主要耦合作用的实时表征支撑受约束运动控制，以接触几何修正减小目标位姿偏差，并明确参考变化、实际阻抗与安装端受力之间的关系。\rev{相关方法将在基座运动条件下通过地面试验验证。}
\subsubsection{\rev{与相关指南的关系}}

\rev{本项目主要对应指南2.2“轻质柔性结构动力学设计与分析”中柔性空间结构在轨组装及刚柔耦合体系力学控制研究，围绕机器人安装界面的动态作用、受约束运动与接触组装开展研究；同时衔接指南2.5关于多源不确定性条件下闭环分析与控制的研究内容。}
\begingroup
\small
\setstretch{1.08}
...
\rev{本项目面向柔性支撑条件下机器人的精确运动与模块组装}，\rev{采用理论分析、数值仿真与地面试验相结合的方法}，研究桁架振动条件下的耦合动力学与运动控制、目标位姿不确定条件下的接触对准与柔顺组装方法，并开展地面试验验证。具体研究内容如下：

\topic{（1）\rev{柔性桁架支撑下机械臂的耦合动力学与运动控制}}
...
\begin{enumerate}[label=\arabic*)]
 ...
 \item \rev{研究给定运动参考下安装端作用力与力矩的建模偏差及驱动约束对实际阻抗响应的影响}，揭示末端运动精度、接触柔顺性与安装端反作用之间的关联，明确不同基座振动、运动速度及末端负载条件下的控制参数选取依据与方法适用范围。
\end{enumerate}

\topic{（2）目标位姿不确定条件下的接触对准与柔顺组装}

\rev{基座运动同时影响接触观测与运动执行，目标估计误差与参考跟踪偏差共同作用于对准精度和接触载荷。以已有接触场配准和约束导纳捕获方法为基础，研究接触观测误差与参考修正对组装响应的影响，探索接触引导与受约束运动控制的协调方法。主要研究内容如下：}
\begin{enumerate}[label=\arabic*)]
 \item 统一接触观测坐标，分析基座测量误差、时序偏差及接触分布对六维配准的局部影响，识别所选接口中约束较弱的位姿方向，为坐标修正与目标限幅提供依据。
 \item \rev{以六维配准与五轴约束导纳为基础}，保留修正通道的零输入保持性质，并独立调节轴向接近运动，研究修正速率、偏移限幅与下层跟踪能力的匹配，减小参考变化引起的接触载荷增量。
 \item \rev{分析目标估计精度的改善在何种条件下能够降低到位误差与接触载荷，为接触引导组装策略及参数选取提供依据。}
\end{enumerate}

\topic{（3）\rev{柔性支撑条件下机器人运动与组装的地面试验验证}}
...
\begin{enumerate}[label=\arabic*)]
 ...
 \item 开展位姿保持与轨迹跟踪试验，通过不同基座振动、运动速度及末端负载条件下的数值仿真与实测响应对照，校核安装端作用力与力矩的动力学描述，\rev{评价所提运动控制方法的运动精度、实际阻抗响应、力矩约束满足情况与实时执行能力}。
...
\rev{面向大型空间结构在轨组装需求}，建立基于安装界面运动与受力的耦合动力学描述，揭示桁架振动、关节驱动约束与末端运动响应之间的关系，形成几何阻抗与动力学优化相结合的运动控制方法；明确典型接口局部接触观测误差与参考修正对组装响应的影响，\rev{研究接触参考修正与下层跟踪能力的协调关系，形成兼顾接口对准精度与接触柔顺性的接触引导组装方法}；依托基座振动模拟与机器人作业实验平台，验证所提动力学描述与控制方法的有效性，\rev{明确其在不同基座振动、末端负载及初始位姿偏差条件下的适用范围。}
...
需要揭示安装端运动与受力向末端位姿响应传递的规律，\rev{阐明给定运动参考下安装端作用力与力矩的建模偏差及受约束执行偏差对实际阻抗特性的共同影响}，建立末端运动精度、接触柔顺性与安装端反作用之间的联系，为柔性桁架振动条件下的受约束运动控制及参数选取提供依据。
...
沿用以作业端为浮动基的倒置动力学模型，将实际安装基座作为倒置运动链的虚拟末端，通过该处的作用力与力矩描述柔性桁架对机械臂的作用。结合关节状态与作业端的绝对位姿、速度测量，确定倒置模型的运动状态，使基座运动对末端的影响能够通过实际运动反馈进入控制过程。在该建模框架下，结合几何阻抗设计与动力学优化：由几何阻抗关系给出作业端参考加速度，利用动力学二次规划（QP）协调末端运动、关节运动与力矩分配。安装端作用力与力矩的计算以现有静力平衡近似为基线，结合安装端运动、作用力与力矩数据校核其在动态工况下的适用性，\rev{研究动态作用的表征及建模偏差对实际控制响应的影响，为模型完善与控制设计提供依据}。

在地面实验平台中，作业机械臂基座与基座运动模拟装置末端固定连接，该连接处对应模型中的实际安装端；作业机械臂末端对应倒置模型的浮动基。模拟装置提供安装端运动激励，作业机械臂的关节力矩由所提控制方法生成。结合实测安装端运动、作用力与力矩，校核安装端受力模型。由此建立在轨桁架与机器人的相互作用、地面基座激励与控制模型之间的对应关系。\rev{拟结合安装端运动与受力分析，明确给定基座激励能够表征的耦合作用及其适用范围。}
...
该更新保证修正量有界，并满足参考端口的离散广义功不等式。\rev{拟研究修正后的位姿参考与下层控制所需速度、加速度参考的衔接，结合局部误差分析，考察修正参数、跟踪误差与参考加速度松弛对到位精度和接触力峰值的影响，为参考修正与执行能力的匹配提供依据。}理论结论限于局部误差传播和参考子系统，整机效果由对照试验评价。
...
对应第一项研究内容，先通过位姿保持与圆轨迹跟踪试验校核倒置动力学模型与运动控制方法，对比数值仿真与实测响应，考察不同基座振动、末端负载及约束激活时的位姿误差、力矩边界与柔顺响应。记录位置均方根误差、旋转测地误差、关节力矩幅值与变化率，以及安装端力和力矩；同时记录控制计算耗时的分布、最大值及超出控制周期的比例，验证所提方法的实时执行能力。同一工况下，保持模拟装置的激励指令、作业机械臂的任务参考、执行时间与驱动约束一致，并核查实际基座运动的一致性，至少进行5次重复试验；参数对照在代表性工况开展。\rev{结合接触扰动下的力—运动响应，比较设计阻抗与实际响应，检验建模偏差及驱动约束对柔顺特性的影响。}

对应第二项研究内容，采用一种典型接口和一种代表性载荷，\rev{将目标固定于独立刚性基准，以考察机器人安装端相对目标运动时的局部组装响应。}先在基座静止时开展基础柔顺控制、仅配准、仅接触参考修正及二者结合的四组对照，再选取少量平移或转动激励重复关键对照。统一初始偏差、执行预算和保持窗口，记录目标估计误差、独立测量的到位误差、达标次数、接触力峰值及安装端受力。工况与判据结合接口容差、测量精度和预实验结果，在正式对照前固定；成功与失败试验均纳入评价，调参数据不计入独立验证。
...
在已有倒置建模基础上，以作业端为浮动基、实际安装端为虚拟末端，将作业端运动、安装端作用力与力矩纳入同一动力学描述，结合几何阻抗设计与动力学优化，协调末端运动响应与关节力矩分配。拟阐明安装端作用力与力矩的建模偏差及驱动约束对实际阻抗响应的共同影响，\rev{形成面向柔性支撑条件下机器人的控制方法}，降低对在线结构力学求解的依赖。
...
\topic{2027年7月至12月}
完成研究内容一的几何阻抗与动力学优化控制设计及初步闭环分析，开展不同基座振动与末端负载条件下的\rev{位姿保持、轨迹跟踪及阻抗响应试验}，明确安装端作用力与力矩的建模偏差和约束激活对运动与阻抗响应的影响；启动研究内容二的基座运动与接触观测同步校核，检验已有接触场在动态工况下的适用性。形成论文初稿1篇、年度进展报告，并与重点实验室开展专题交流。
