...
围绕这一需求，现有建模方法主要关注动态特征保留与模型规模控制。模态缩减及模型降阶通过保留主要变形特征降低系统维数，为复杂柔性多体系统的分析提供了有效途径\cite{sonneville2021,min2026}；时变动力学边界研究则表明，模型参数与约束条件需要随作业状态调整，固定边界下的简化结果难以直接覆盖整个组装过程\cite{ni2024}。从方法特点看，模型简化的效果取决于被保留的动态特征是否与关注的作业响应相匹配。面向末端精度与接口受力的控制模型，需要进一步关注结构运动如何通过安装端传递至机械臂，而非仅以整体结构响应的近似精度评价模型。

耦合关系还受到结构尺度与接触阶段的影响。超大型结构的组装分析显示，科里奥利效应等空间扰动会影响控制需求与振动响应，其影响随结构尺度和作业条件变化\cite{yang2023}；柔性模块接触建模则将冲击、结构变形与机器人运动联系起来，揭示接近速度和控制参数对接触响应的作用\cite{wucontact2026}。由此可见，自由转运与接触组装对模型的要求有所不同：前者侧重运动与结构振动的传播，后者还需描述接触约束建立及载荷突变
...
对应三项研究内容，形成包含倒置动力学模型、几何阻抗设计与动力学优化的机器人运动控制程序1套、典型接口的接触配准与参考修正模块1套，并建立地面实验平台的基座振动工况与试验流程，形成同步测量数据集；完成振动扰动下的机器人运动与典型接口组装地面原理验证，提交技术总结报告和试验测试报告各1份。围绕新增研究发表或录用学术论文3篇，培养参与研究的研究生3名。
